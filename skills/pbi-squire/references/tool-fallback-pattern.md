...
2. **If tool available (Developer Edition):**
...
| `pbir_visual_editor.py` | Execute visual edit plan | Parse JSON, use Edit tool | Developer-only (visual editing) |
...
6. [ ] Update Prerequisites section to mark Python as "Optional (Developer)"
...
Some tools are **Developer-only with no Core fallback** because they provide optional analytics:
